...
function makeApi(
  ...
  getResourceRecognition: jest.Mock<
  ...
  return new EvernoteAPI(
    client as any,
    { noteStoreUrl: "https://example/notestore" } as any,
  );
}
...
describe("EvernoteAPI.extractPdfTextFromResource", () => {
  beforeEach(() => {
    ...
    MockedPDFParse.mockImplementation(() => ({
      getText: jest
        ...
        .mockResolvedValue({ text: "extracted text" }),
    }));
  });
  ...
  it("reuses a prefetched resource body and does not download again", async () => {
    ...
    const result = await makeApi(getResource).extractPdfTextFromResource(
      "g3",
      prefetched,
    );
  ...
  it("returns a generic failure message when the fetch throws", async () => {
    ...
    expect(result).toBe(
      "[PDF text extraction failed — could not retrieve the attachment]",
    );
  });
});
...
describe("EvernoteAPI.extractResourceText", () => {
  ...
  it("falls back to Evernote recognition text when a PDF has no text layer", async () => {
    ...
    const result = await makeApi(getResource).extractResourceText(
      "pdf-guid",
      prefetched,
    );
  ...
  it("extracts OCR text from image recognition data without downloading the body", async () => {
    ...
    const result = await makeApi(
      getResource,
      getResourceRecognition,
    ).extractResourceText("image-guid");
  ...
  it("reuses prefetched recognition data and does not download the resource again", async () => {
    ...
    const result = await makeApi(
      getResource,
      getResourceRecognition,
    ).extractResourceText("image-guid", prefetched);
  ...
  it("fetches OCR when prefetched recognition has metadata only", async () => {
    const getResource = jest.fn<(...args: any[]) => Promise<any>>();
    const getResourceRecognition = jest.fn<(...args: any[]) => Promise<any>>()
      .mockResolvedValue(`
      <recoIndex>
        <item x="1" y="2" w="3" h="4"><t w="95">Metadata OCR</t></item>
      </recoIndex>
    `);
    const prefetched = {
      mime: "image/jpeg",
    ...
    expect(result).toBe("Metadata OCR");
    expect(getResourceRecognition).toHaveBeenCalledWith("image-guid");
    expect(getResource).not.toHaveBeenCalled();
  });

  it("propagates recognition API failures instead of masking them", async () => {
    const getResource = jest
      .fn<(...args: any[]) => Promise<any>>()
      .mockResolvedValue({ mime: "image/png" });
    ...
    await expect(
      makeApi(getResource, getResourceRecognition).extractResourceText(
        "image-guid",
      ),
