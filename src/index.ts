...
const tools: Tool[] = [
  ...
  },
  {
    ...
    description: 'Get a specific note by its GUID. Attachment text from PDFs and image OCR is extracted and included by default; set includeAttachmentText: false to skip it.',
    inputSchema: {
      type: 'object',
      properties: {
        guid: {
  ...
  },
  {
    ...
    description: 'Extract plain text from a resource attachment. Supports PDF text layers and Evernote OCR recognition for images. Returns the text content or an explanatory message if extraction is unavailable.',
    inputSchema: {
      type: 'object',
      properties: {
        resourceGuid: {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'evernote_get_note': {
        const { guid, includeContent = true, includeAttachmentText = true } = validatedArgs;
        // Fetch resources whenever attachment text/resource metadata is wanted, independent of includeContent.
        const note = await evernoteApi.getNote(guid, includeContent, includeAttachmentText);
